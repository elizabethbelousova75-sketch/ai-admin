...
const BLOCKS: Record<BlockType, { label: string; icon: string; variants: string[]; defaultContent: Record<string, string> }> = {
  quiz: {
    label: 'Квиз', icon: '📋',
    variants: ['Попап по кнопке'],
    defaultContent: { quiz_id: '', button_text: 'Пройти квиз', button_color: '#3b82f6' }
  },
  hero: {
...
export default function SitesPage() {
  ...
  useEffect(() => { loadSites(); loadQuizList() }, [])

  const loadQuizList = async () => {
    try {
      const res = await fetch('/api/quizzes')
      const data = await res.json()
      if (Array.isArray(data)) setQuizList(data.map((q:any) => ({ id: q.id, title: q.title })))
    } catch {}
  }
  ...
  const renderBlock = (block: Block) => {
    ...
    // ── КВИЗ ──────────────────────────────────────────────────────────────────
    if (block.type === 'quiz') {
      const origin = typeof window !== 'undefined' ? window.location.origin : 'https://bfl-ai-admin.vercel.app'
      const quizName = quizList.find(q => q.id === c.quiz_id)?.title
      return (
        <div style={{ padding: '40px', background: 'linear-gradient(135deg, #1a1040, #0f2040)', borderRadius: '8px', textAlign: 'center', border: '1px solid rgba(99,102,241,0.3)' }}>
          <div style={{ fontSize: '40px', marginBottom: '12px' }}>📋</div>
          {c.quiz_id ? (
            <div>
              <p style={{ color: '#a5b4fc', marginBottom: '4px', fontSize: '15px', fontWeight: '600' }}>{quizName || 'Квиз'}</p>
              <p style={{ color: '#64748b', marginBottom: '16px', fontSize: '13px' }}>Квиз подключён ✅</p>
              <div style={{ background: 'rgba(0,0,0,0.3)', borderRadius: '8px', padding: '10px 14px', marginBottom: '16px', fontFamily: 'monospace', fontSize: '11px', color: '#94a3b8', textAlign: 'left', wordBreak: 'break-all' }}>
                {`<script src="${origin}/quiz/${c.quiz_id}/widget.js"></script>`}
              </div>
              <button style={{ padding: '10px 24px', background: c.button_color || '#3b82f6', color: 'white', border: 'none', borderRadius: '8px', fontWeight: '600', fontSize: '14px', cursor: 'pointer' }}>
                {c.button_text || 'Пройти квиз'}
              </button>
            </div>
          ) : (
            <p style={{ color: '#64748b', fontSize: '14px' }}>Выберите квиз в панели редактора →</p>
          )}
        </div>
      )
    }
  ...
  const fieldLabels: Record<string, string> = {
    ...
    quiz_id: 'Выбрать квиз', button_text: 'Текст кнопки квиза', button_color: 'Цвет кнопки'
  }
  ...
  if (view === 'pages' && activeSite) return (
    <div style={{ background: '#f8fafc', minHeight: '100vh', margin: '-28px', padding: '0' }}>
      <div style={{ background: 'white', borderBottom: '1px solid #e2e8f0', padding: '0 32px', display: 'flex', alignItems: 'center', gap: '0', height: '52px' }}>
        <button onClick={() => setView('list')} style={{ display: 'flex', alignItems: 'center', gap: '8px', padding: '0 20px 0 0', background: 'none', border: 'none', cursor: 'pointer', color: '#64748b', fontSize: '14px', borderRight: '1px solid #e2e8f0', height: '100%' }}>← Назад</button>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
          ...
          <button onClick={() => setView('settings')} style={{ display: 'flex', alignItems: 'center', gap: '6px', padding: '7px 16px', background: '#f1f5f9', border: '1px solid #e2e8f0', borderRadius: '6px', cursor: 'pointer', fontSize: '13px', color: '#64748b' }}>⚙️ Настройки</button>
          <button onClick={() => setShowNewPage(true)} style={{ display: 'flex', alignItems: 'center', gap: '6px', padding: '7px 16px', background: '#3b82f6', border: 'none', borderRadius: '6px', cursor: 'pointer', fontSize: '13px', color: 'white', fontWeight: '600' }}>+ Создать страницу</button>
        </div>
      </div>
        ...
        <div style={{ background: 'white', borderRadius: '12px', border: '1px solid #e2e8f0', overflow: 'hidden' }}>
          {activeSite.pages.map((page, idx) => (
            <div key={page.id} style={{ display: 'flex', alignItems: 'center', padding: '16px 20px', borderBottom: idx < activeSite.pages.length - 1 ? '1px solid #f1f5f9' : 'none', transition: 'background 0.15s' }}
              ...
              <div onClick={() => { setActivePage(page); setView('editor') }} style={{ width: '72px', height: '54px', background: page.blocks?.length ? 'linear-gradient(135deg, #667eea, #764ba2)' : '#f1f5f9', borderRadius: '6px', marginRight: '20px', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', flexShrink: 0, border: '1px solid #e2e8f0', overflow: 'hidden' }}>
                {page.blocks?.length ? (
              ...
              <div style={{ display: 'flex', gap: '8px', alignItems: 'center' }}>
                <button onClick={() => { setActivePage(page); setView('editor') }} style={{ padding: '6px 14px', background: '#3b82f6', border: 'none', borderRadius: '6px', color: 'white', cursor: 'pointer', fontSize: '13px', fontWeight: '600' }}>Редактировать</button>
                {activeSite.pages.length > 1 && (
                  <button onClick={() => deletePage(page.id)} style={{ padding: '6px 10px', background: 'none', border: '1px solid #fee2e2', borderRadius: '6px', color: '#ef4444', cursor: 'pointer', fontSize: '13px' }}>🗑️</button>
                )}
              </div>
            </div>
          ))}
  ...
  if (view === 'editor' && activeSite && activePage) return (
    <div style={{ display: 'flex', height: '100vh', overflow: 'hidden', marginLeft: '-28px', marginTop: '-28px' }}>
      <div style={{ width: '260px', background: '#1e293b', borderRight: '1px solid #334155', display: 'flex', flexDirection: 'column', flexShrink: 0 }}>
        ...
        <div style={{ padding: '12px 16px', borderBottom: '1px solid #e2e8f0' }}>
          <button onClick={() => setShowBlockPicker(true)} style={{ width: '100%', padding: '10px', background: '#3b82f6', border: 'none', borderRadius: '6px', color: 'white', cursor: 'pointer', fontWeight: '600', fontSize: '13px' }}>+ Добавить блок</button>
        </div>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
              {Object.entries(editingBlock.content).map(([field, value]) => (
                <div key={field}>
                  ...
                  {field === 'quiz_id' ? (
                    <select value={value} onChange={e => updateBlock(editingBlock.id, field, e.target.value)}
                      style={{ width: '100%', padding: '8px 10px', background: 'white', border: '1px solid #e2e8f0', borderRadius: '6px', color: '#0f172a', fontSize: '12px' }}>
