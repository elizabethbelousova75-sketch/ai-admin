...
type BlockType = 'hero' | 'text' | 'features' | 'cta' | 'contacts' | 'html' | 'gallery' | 'form' | 'reviews' | 'video' | 'pricing' | 'stats' | 'faq' | 'team'
...
export default function SitesPage() {
  ...
  const renderBlock = (block: Block) => {
    ...
    if (block.type === 'hero') {
      ...
      if (v === 1) return (
        <div style={{ background: '#0f172a', padding: '80px 40px', textAlign: 'center', borderRadius: '8px', border: '1px solid #334155' }}>
      ...
      if (v === 2) return (
        <div style={{ background: 'white', padding: '80px 40px', textAlign: 'left', borderRadius: '8px', display: 'flex', alignItems: 'center', gap: '40px' }}>
          <div style={{ flex: 1 }}>
            ...
            <a href={c.buttonUrl || '#'} style={{ display: 'inline-block', background: '#0f172a', color: 'white', padding: '14px 32px', borderRadius: '8px', fontWeight: 'bold', textDecoration: 'none' }}>{c.buttonText}</a>
          </div>
    ...
    if (block.type === 'text') {
      if (v === 0) return (
        <div style={{ padding: '60px 40px', background: '#1e293b', borderRadius: '8px' }}>
          <h2 style={{ fontSize: '32px', fontWeight: 'bold', color: '#f1f5f9', marginBottom: '20px' }}>{c.title}</h2>
          <p style={{ color: '#94a3b8', lineHeight: '1.8', fontSize: '16px' }}>{c.text}</p>
        </div>
      )
      if (v === 1) return (
        <div style={{ padding: '60px 40px', background: '#0f172a', borderRadius: '8px', display: 'flex', gap: '40px', alignItems: 'flex-start' }}>
          <div style={{ fontSize: '48px' }}>💡</div>
          <div>
            <h2 style={{ fontSize: '28px', fontWeight: 'bold', color: '#f1f5f9', marginBottom: '16px' }}>{c.title}</h2>
            <p style={{ color: '#94a3b8', lineHeight: '1.8', fontSize: '16px' }}>{c.text}</p>
          </div>
        </div>
      )
      if (v === 2) return (
        <div style={{ padding: '60px 40px', background: '#1e293b', borderRadius: '8px', borderLeft: '4px solid #3b82f6' }}>
          <div style={{ fontSize: '32px', color: '#3b82f6', marginBottom: '16px' }}>"</div>
          <p style={{ color: '#f1f5f9', lineHeight: '1.8', fontSize: '20px', fontStyle: 'italic', marginBottom: '16px' }}>{c.text}</p>
    ...
    if (block.type === 'features') {
      if (v === 0) return (
        <div style={{ padding: '60px 40px', background: '#0f172a', borderRadius: '8px' }}>
          <h2 style={{ fontSize: '32px', fontWeight: 'bold', color: '#f1f5f9', textAlign: 'center', marginBottom: '40px' }}>{c.title}</h2>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '24px' }}>
            {[[c.f1, c.d1, c.icon1], [c.f2, c.d2, c.icon2], [c.f3, c.d3, c.icon3]].map(([f, d, icon], i) => (
              <div key={i} style={{ background: '#1e293b', padding: '28px', borderRadius: '8px', textAlign: 'center' }}>
                <div style={{ fontSize: '36px', marginBottom: '16px' }}>{icon}</div>
                <h3 style={{ color: '#f1f5f9', fontWeight: '600', marginBottom: '8px', fontSize: '16px' }}>{f}</h3>
        ...
        <div style={{ padding: '60px 40px', background: '#1e293b', borderRadius: '8px' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: '20px' }}>
            {[[c.f1, c.d1, c.icon1], [c.f2, c.d2, c.icon2], [c.f3, c.d3, c.icon3], [c.f4, c.d4, c.icon4]].map(([f, d, icon], i) => (
              <div key={i} style={{ display: 'flex', gap: '16px', alignItems: 'flex-start', padding: '20px', background: '#0f172a', borderRadius: '8px' }}>
                <div style={{ fontSize: '28px', flexShrink: 0 }}>{icon}</div>
                <div>
                  <h3 style={{ color: '#f1f5f9', fontWeight: '600', marginBottom: '6px' }}>{f}</h3>
                  <p style={{ color: '#64748b', fontSize: '14px', lineHeight: '1.6' }}>{d}</p>
                </div>
              </div>
            ))}
      ...
      if (v === 2) return (
        <div style={{ padding: '60px 40px', background: '#0f172a', borderRadius: '8px' }}>
          <h2 style={{ fontSize: '32px', fontWeight: 'bold', color: '#f1f5f9', marginBottom: '32px' }}>{c.title}</h2>
          <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
            {[[c.f1, c.d1], [c.f2, c.d2], [c.f3, c.d3], [c.f4, c.d4]].map(([f, d], i) => (
              <div key={i} style={{ display: 'flex', alignItems: 'center', gap: '16px', padding: '16px', background: '#1e293b', borderRadius: '8px' }}>
                ...
                <div>
                  <span style={{ color: '#f1f5f9', fontWeight: '600' }}>{f}</span>
    ...
    if (block.type === 'reviews') {
        ...
        <div style={{ padding: '60px 40px', background: '#0f172a', borderRadius: '8px' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '20px' }}>
            {[[c.name1, c.text1, c.rating1], [c.name2, c.text2, c.rating2], [c.name3, c.text3, c.rating3]].map(([name, text, rating], i) => (
              <div key={i} style={{ background: '#1e293b', padding: '24px', borderRadius: '8px', border: '1px solid #334155' }}>
                <div style={{ color: '#f59e0b', fontSize: '18px', marginBottom: '12px' }}>{'⭐'.repeat(parseInt(rating || '5'))}</div>
                <p style={{ color: '#e2e8f0', fontSize: '15px', lineHeight: '1.7', marginBottom: '16px', fontStyle: 'italic' }}>"{text}"</p>
                <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                  ...
                  <span style={{ color: '#f1f5f9', fontWeight: '600', fontSize: '14px' }}>{name}</span>
                </div>
              </div>
            ))}
          </div>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '24px' }}>
            {[[c.name1, c.text1, c.rating1], [c.name2, c.text2, c.rating2], [c.name3, c.text3, c.rating3]].map(([name, text, rating], i) => (
              <div key={i} style={{ borderLeft: '3px solid #3b82f6', paddingLeft: '24px' }}>
                <p style={{ color: '#e2e8f0', fontSize: '18px', lineHeight: '1.7', marginBottom: '12px', fontStyle: 'italic' }}>"{text}"</p>
      ...
      if (v === 2) return (
        <div style={{ padding: '60px 40px', background: '#0f172a', borderRadius: '8px' }}>
          <h2 style={{ fontSize: '32px', fontWeight: 'bold', color: '#f1f5f9', marginBottom: '32px' }}>{c.title}</h2>
          {[[c.name1, c.text1, c.rating1], [c.name2, c.text2, c.rating2], [c.name3, c.text3, c.rating3]].map(([name, text, rating], i) => (
            <div key={i} style={{ display: 'flex', gap: '16px', padding: '20px 0', borderBottom: '1px solid #1e293b' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '20px' }}>
            {plans.map((plan, i) => (
              <div key={i} style={{ background: plan.popular ? '#1e3a8a' : '#1e293b', padding: '32px 24px', borderRadius: '8px', border: plan.popular ? '2px solid #3b82f6' : '1px solid #334155', position: 'relative', textAlign: 'center' }}>
                ...
                <h3 style={{ color: '#f1f5f9', fontSize: '18px', fontWeight: '600', marginBottom: '16px' }}>{plan.name}</h3>
                <div style={{ marginBottom: '24px' }}>
                  <span style={{ fontSize: '36px', fontWeight: 'bold', color: '#f1f5f9' }}>{plan.price}</span>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: '20px', maxWidth: '700px', margin: '0 auto' }}>
            {plans.slice(0, 2).map((plan, i) => (
              <div key={i} style={{ background: i === 1 ? 'linear-gradient(135deg, #1e3a8a, #3b82f6)' : '#0f172a', padding: '32px 24px', borderRadius: '12px', textAlign: 'center' }}>
                <h3 style={{ color: '#f1f5f9', fontSize: '20px', fontWeight: '700', marginBottom: '8px' }}>{plan.name}</h3>
                <div style={{ fontSize: '40px', fontWeight: 'bold', color: 'white', marginBottom: '20px' }}>{plan.price}<span style={{ fontSize: '16px', color: '#94a3b8' }}> руб</span></div>
                {plan.features.filter(Boolean).map((f, j) => (
                  <div key={j} style={{ color: '#e2e8f0', fontSize: '14px', marginBottom: '8px' }}>✓ {f}</div>
                ))}
        ...
        <div style={{ padding: '60px 40px', background: '#0f172a', borderRadius: '8px' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '24px' }}>
            {items.slice(0, 3).map(([n, l], i) => (
              <div key={i} style={{ background: '#1e293b', padding: '32px', borderRadius: '8px', textAlign: 'center', border: '1px solid #334155' }}>
                <div style={{ fontSize: '48px', fontWeight: '800', color: '#3b82f6', marginBottom: '8px' }}>{n}</div>
                <div style={{ color: '#94a3b8', fontSize: '15px' }}>{l}</div>
              </div>
            ))}
          </div>
        </div>
    ...
    if (block.type === 'faq') {
        ...
        <div style={{ padding: '60px 40px', background: '#0f172a', borderRadius: '8px' }}>
          ...
          <div style={{ maxWidth: '700px', margin: '0 auto', display: 'flex', flexDirection: 'column', gap: '12px' }}>
            {[[c.q1, c.a1], [c.q2, c.a2], [c.q3, c.a3], [c.q4, c.a4]].map(([q, a], i) => (
              <div key={i} style={{ background: '#1e293b', borderRadius: '8px', overflow: 'hidden', border: '1px solid #334155' }}>
                <div style={{ padding: '18px 20px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                  <span style={{ color: '#f1f5f9', fontWeight: '600', fontSize: '15px' }}>{q}</span>
                  <span style={{ color: '#3b82f6', fontSize: '20px' }}>+</span>
                </div>
                <div style={{ padding: '0 20px 18px', color: '#94a3b8', fontSize: '14px', lineHeight: '1.7' }}>{a}</div>
              </div>
            ))}
          </div>
        </div>
      ...
      if (v === 1) return (
        <div style={{ padding: '60px 40px', background: '#1e293b', borderRadius: '8px' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: '20px' }}>
            {[[c.q1, c.a1], [c.q2, c.a2], [c.q3, c.a3], [c.q4, c.a4]].map(([q, a], i) => (
              <div key={i} style={{ padding: '24px', background: '#0f172a', borderRadius: '8px' }}>
    ...
    if (block.type === 'team') {
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '24px' }}>
            {[[c.name1, c.role1, c.img1], [c.name2, c.role2, c.img2], [c.name3, c.role3, c.img3]].map(([name, role, img], i) => (
              <div key={i} style={{ background: '#1e293b', padding: '28px', borderRadius: '8px', textAlign: 'center' }}>
                ...
                <h3 style={{ color: '#f1f5f9', fontWeight: '600', marginBottom: '4px' }}>{name}</h3>
      ...
      if (v === 1) return (
        <div style={{ padding: '60px 40px', background: '#1e293b', borderRadius: '8px' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
            {[[c.name1, c.role1, c.img1], [c.name2, c.role2, c.img2], [c.name3, c.role3, c.img3]].map(([name, role, img], i) => (
              <div key={i} style={{ display: 'flex', gap: '20px', alignItems: 'center', padding: '20px', background: '#0f172a', borderRadius: '8px' }}>
    ...
    if (block.type === 'gallery') {
      ...
      if (v === 0) return (
        <div style={{ padding: '60px 40px', background: '#0f172a', borderRadius: '8px' }}>
          <h2 style={{ fontSize: '32px', fontWeight: 'bold', color: '#f1f5f9', textAlign: 'center', marginBottom: '32px' }}>{c.title}</h2>
      ...
      if (v === 2) return (
        <div style={{ padding: '40px 0', background: '#0f172a', borderRadius: '8px', overflow: 'hidden' }}>
          <h2 style={{ fontSize: '32px', fontWeight: 'bold', color: '#f1f5f9', textAlign: 'center', marginBottom: '24px', padding: '0 40px' }}>{c.title}</h2>
    ...
    if (block.type === 'form') {
      if (v === 0) return (
        <div style={{ padding: '60px 40px', background: '#1e293b', borderRadius: '8px' }}>
          <div style={{ maxWidth: '560px', margin: '0 auto', textAlign: 'center' }}>
            <h2 style={{ fontSize: '32px', fontWeight: 'bold', color: '#f1f5f9', marginBottom: '12px' }}>{c.title}</h2>
            <p style={{ color: '#64748b', marginBottom: '32px' }}>{c.subtitle}</p>
            <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
              <input placeholder={c.namePlaceholder} style={{ padding: '12px 16px', background: '#0f172a', border: '1px solid #334155', borderRadius: '8px', color: '#f1f5f9', fontSize: '14px' }} />
              <input placeholder={c.phonePlaceholder} style={{ padding: '12px 16px', background: '#0f172a', border: '1px solid #334155', borderRadius: '8px', color: '#f1f5f9', fontSize: '14px' }} />
              <textarea placeholder={c.messagePlaceholder} rows={3} style={{ padding: '12px 16px', background: '#0f172a', border: '1px solid #334155', borderRadius: '8px', color: '#f1f5f9', fontSize: '14px', resize: 'none' }} />
      ...
      if (v === 2) return (
        <div style={{ padding: '40px', background: '#0f172a', borderRadius: '8px' }}>
          <div style={{ display: 'flex', gap: '40px', alignItems: 'center' }}>
            <div style={{ flex: 1 }}>
              <h2 style={{ fontSize: '28px', fontWeight: 'bold', color: '#f1f5f9', marginBottom: '8px' }}>{c.title}</h2>
              <p style={{ color: '#64748b', marginBottom: '0' }}>{c.subtitle}</p>
            </div>
            <div style={{ flex: 1, display: 'flex', gap: '12px' }}>
              <input placeholder={c.phonePlaceholder} style={{ flex: 1, padding: '12px 16px', background: '#1e293b', border: '1px solid #334155', borderRadius: '8px', color: '#f1f5f9', fontSize: '14px' }} />
    ...
    if (block.type === 'video') {
      if (v === 0) return (
        <div style={{ padding: '60px 40px', background: '#0f172a', borderRadius: '8px', textAlign: 'center' }}>
          <h2 style={{ fontSize: '32px', fontWeight: 'bold', color: '#f1f5f9', marginBottom: '12px' }}>{c.title}</h2>
          <p style={{ color: '#64748b', marginBottom: '32px' }}>{c.subtitle}</p>
      ...
      if (v === 1) return (
        <div style={{ padding: '40px', background: '#1e293b', borderRadius: '8px', display: 'flex', gap: '40px', alignItems: 'center' }}>
          <div style={{ flex: 1, borderRadius: '8px', overflow: 'hidden', aspectRatio: '16/9' }}>
            <iframe src={c.videoUrl} style={{ width: '100%', height: '100%', border: 'none' }} allowFullScreen />
          </div>
          <div style={{ flex: 1 }}>
            <h2 style={{ fontSize: '28px', fontWeight: 'bold', color: '#f1f5f9', marginBottom: '12px' }}>{c.title}</h2>
    ...
    if (block.type === 'cta') {
      ...
      if (v === 1) return (
        <div style={{ background: '#0f172a', padding: '60px 40px', textAlign: 'center', borderRadius: '8px', border: '1px solid #334155' }}>
          <h2 style={{ fontSize: '32px', fontWeight: 'bold', color: '#f1f5f9', marginBottom: '12px' }}>{c.title}</h2>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
            {[['📞', c.phone], ['✉️', c.email], ['📍', c.address], ['🕐', c.workTime]].map(([icon, val], i) => (
              <div key={i} style={{ display: 'flex', gap: '12px', alignItems: 'center' }}>
                ...
                <span style={{ color: '#e2e8f0', fontSize: '16px' }}>{val}</span>
              </div>
            ))}
          </div>
        </div>
        ...
        <div style={{ padding: '60px 40px', background: '#0f172a', borderRadius: '8px' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '20px' }}>
            {[['📞', 'Телефон', c.phone], ['✉️', 'Email', c.email], ['📍', 'Адрес', c.address]].map(([icon, label, val], i) => (
              <div key={i} style={{ background: '#1e293b', padding: '24px', borderRadius: '8px', textAlign: 'center' }}>
    ...
    if (block.type === 'html') return (
      <div style={{ position: 'relative' }}>
        <div style={{ position: 'absolute', top: '8px', right: '8px', background: '#334155', color: '#94a3b8', fontSize: '11px', padding: '2px 8px', borderRadius: '4px', zIndex: 1 }}>💻 HTML</div>
        <div dangerouslySetInnerHTML={{ __html: c.code }} />
      </div>
    )

    return <div style={{ padding: '40px', background: '#1e293b', borderRadius: '8px', color: '#64748b', textAlign: 'center' }}>Блок: {block.type} (вариант {v + 1})</div>
  }
          ...
          {sites.map(s => (
            <div key={s.id} style={{ background: '#1e293b', border: '1px solid #334155', borderRadius: '12px', padding: '20px' }}>
              ...
              <div style={{ display: 'flex', gap: '6px', marginBottom: '14px', flexWrap: 'wrap' }}>
                <span style={{ fontSize: '11px', padding: '3px 8px', borderRadius: '20px', background: s.status === 'published' ? '#064e3b' : '#0f172a', color: s.status === 'published' ? '#34d399' : '#94a3b8', border: '1px solid #334155' }}>{s.status === 'published' ? '● Опубликован' : '○ Черновик'}</span>
  ...
  if (view === 'settings' && activeSite) return (
    <div style={{ maxWidth: '640px' }}>
      ...
      <div style={{ background: '#1e293b', border: '1px solid #334155', borderRadius: '12px', padding: '24px', marginBottom: '16px' }}>
        ...
        {activeSite.domain && (
          <div style={{ background: '#0f172a', border: '1px solid #334155', borderRadius: '8px', padding: '16px' }}>
            <p style={{ fontSize: '12px', color: '#94a3b8', marginBottom: '10px', fontWeight: '600' }}>DNS записи:</p>
            <div style={{ background: '#1e293b', padding: '8px 12px', borderRadius: '6px', marginBottom: '6px', display: 'flex', gap: '16px' }}>
              <span style={{ fontSize: '12px', color: '#60a5fa', width: '50px', fontWeight: '600' }}>A</span>
              <span style={{ fontSize: '12px', color: '#f1f5f9', width: '40px' }}>@</span>
              <span style={{ fontSize: '12px', color: '#34d399', fontFamily: 'monospace' }}>76.76.21.21</span>
            </div>
            <div style={{ background: '#1e293b', padding: '8px 12px', borderRadius: '6px', display: 'flex', gap: '16px' }}>
              <span style={{ fontSize: '12px', color: '#60a5fa', width: '50px', fontWeight: '600' }}>CNAME</span>
              <span style={{ fontSize: '12px', color: '#f1f5f9', width: '40px' }}>www</span>
      ...
      <div style={{ background: '#1e293b', border: '1px solid #334155', borderRadius: '12px', padding: '24px', marginBottom: '16px' }}>
        ...
        <input value={activeSite.yandex_metrika} onChange={e => setActiveSite({ ...activeSite, yandex_metrika: e.target.value })} placeholder="Номер счётчика: 12345678" style={{ width: '100%', padding: '10px 12px', background: '#0f172a', border: '1px solid #334155', borderRadius: '6px', color: '#f1f5f9', fontSize: '14px' }} />
      </div>
      <div style={{ background: '#1e293b', border: '1px solid #334155', borderRadius: '12px', padding: '24px', marginBottom: '16px' }}>
        ...
        <input value={activeSite.amo_token} onChange={e => setActiveSite({ ...activeSite, amo_token: e.target.value })} placeholder="API токен" type="password" style={{ width: '100%', padding: '10px 12px', background: '#0f172a', border: '1px solid #334155', borderRadius: '6px', color: '#f1f5f9', fontSize: '14px', marginBottom: '14px' }} />
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px' }}>
          <input value={activeSite.amo_pipeline_id} onChange={e => setActiveSite({ ...activeSite, amo_pipeline_id: e.target.value })} placeholder="ID воронки" style={{ padding: '10px 12px', background: '#0f172a', border: '1px solid #334155', borderRadius: '6px', color: '#f1f5f9', fontSize: '14px' }} />
          <input value={activeSite.amo_status_id} onChange={e => setActiveSite({ ...activeSite, amo_status_id: e.target.value })} placeholder="ID статуса" style={{ padding: '10px 12px', background: '#0f172a', border: '1px solid #334155', borderRadius: '6px', color: '#f1f5f9', fontSize: '14px' }} />
        </div>
      </div>
  ...
  if (view === 'editor' && activeSite && activePage) return (
    <div style={{ display: 'flex', height: '100vh', overflow: 'hidden', marginLeft: '-28px', marginTop: '-28px' }}>
        ...
        <div style={{ flex: 1, overflowY: 'auto', padding: '12px 16px' }}>
          ...
          {activePage.blocks?.map(block => (
            <div key={block.id} onClick={() => setEditingBlock(editingBlock?.id === block.id ? null : block)} style={{ padding: '7px 10px', background: editingBlock?.id === block.id ? '#1e3a8a' : '#1e293b', border: `1px solid ${editingBlock?.id === block.id ? '#3b82f6' : '#334155'}`, borderRadius: '6px', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '4px' }}>
              <span style={{ fontSize: '12px', color: '#e2e8f0' }}>{BLOCKS[block.type]?.icon} {BLOCKS[block.type]?.label}</span>
      ...
      {editingBlock && !preview && (
        <div style={{ width: '300px', background: '#0f172a', borderLeft: '1px solid #1e293b', overflowY: 'auto' }}>
          <div style={{ padding: '16px', borderBottom: '1px solid #1e293b', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            <h3 style={{ fontSize: '14px', fontWeight: '600', color: '#f1f5f9' }}>{BLOCKS[editingBlock.type]?.icon} {BLOCKS[editingBlock.type]?.label}</h3>
              ...
              {Object.entries(editingBlock.content).map(([field, value]) => (
                <div key={field}>
                  ...
                  {field === 'text' || field === 'code' || field.startsWith('a') ? (
                    <textarea value={value} onChange={e => updateBlock(editingBlock.id, field, e.target.value)} rows={field === 'code' ? 6 : 3} style={{ width: '100%', padding: '8px 10px', background: '#1e293b', border: '1px solid #334155', borderRadius: '6px', color: '#f1f5f9', fontSize: '12px', resize: 'vertical', fontFamily: field === 'code' ? 'monospace' : 'inherit' }} />
                  ) : (
                    <input value={value} onChange={e => updateBlock(editingBlock.id, field, e.target.value)} style={{ width: '100%', padding: '8px 10px', background: '#1e293b', border: '1px solid #334155', borderRadius: '6px', color: '#f1f5f9', fontSize: '12px' }} />
                  )}
                </div>
              ))}
            </div>
      ...
      {showBlockPicker && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.8)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 200 }}>
          <div style={{ background: '#1e293b', borderRadius: '16px', padding: '32px', width: '700px', maxHeight: '80vh', overflowY: 'auto', border: '1px solid #334155' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '24px' }}>
              <h2 style={{ fontSize: '20px', fontWeight: 'bold', color: '#f1f5f9' }}>Выберите блок</h2>
                ...
                <div key={type}>
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', gap: '4px' }}>
                    {info.variants.map((variant, i) => (
                      <button key={i} onClick={() => addBlock(type, i)} style={{ padding: '8px 12px', background: '#0f172a', border: '1px solid #334155', borderRadius: '6px', color: '#e2e8f0', cursor: 'pointer', fontSize: '12px', textAlign: 'left', transition: 'all 0.15s' }}
                        onMouseOver={e => { e.currentTarget.style.borderColor = '#3b82f6'; e.currentTarget.style.background = '#1e3a8a' }}
